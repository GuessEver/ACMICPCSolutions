...
double query(int p, int l, int r, int a, int b)
{
	...
	if(b > mid) res += query(p*2+1, mid, r, a, b);
	pushUp(p, l, mid, r);
	return res;
}

void change(int p, int l, int r, int a, int b, double A, double D)
{
	if(a <= l && b >= r)
	{
		add[p].A += A;
	...
	int mid = l + (r - l) / 2;
	pushDown(p, l, mid, r);
	if(b <= mid) change(p*2, l, mid, a, b, A, D);
	else if(a >= mid) change(p*2+1, mid, r, a, b, A, D);
	else {
		change(p*2, l, mid, a, mid, A, D);
		change(p*2+1, mid, r, mid, b, A + (P[mid] - P[a]) * D, D);
	}
	pushUp(p, l, mid, r);
}
...
void Solve()
{
	...
	for(int q = 1; q <= m; q++)
	{
		if(que[q].op == 'Q')
		{
			printf("%.3f\n", query(1, 1, n, que[q].a[0][0], que[q].a[0][1]));
		}
		else {
